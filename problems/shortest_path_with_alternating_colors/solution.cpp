...
public:
    vector<int> shortestAlternatingPaths(int n, vector<vector<int>>& red_edges, vector<vector<int>>& blue_edges) {
        ...
        // for n nodes, {distance vis red node, distance via blue node}
        
        
        queue<pair<int, int>> q;
        
        q.push(make_pair(0, 0));  // 0 th node , with red color
        q.push(make_pair(0, 1));  // 0 th node , with blue color
        ...
        while(!q.empty()){
            ...
            for(auto i: graph[node]){
                ...
                if(distances[nodee][edgcoll] == INT_MAX && edgcoll != edgcol){
                    ...
                    q.push(make_pair(nodee, edgcoll));  // different color
                }
            }
        }
