...
vector <int> countsmaller(vector <int> &nums){
    ...
    long long goodTriplets(vector<int>& A, vector<int>& B) {
        ...
        for(auto &val : A){
            val = umap[val];                 // replacing elements of A with their indices in B.
        }
        vector <int> ans = countsmaller(A);  // this contains no.of elements smaller to this to the right.
        long long int res =  0;
        for(int i =0; i < A.size() ; i++){
            long long  rightelements  = n - i -1 ;
            long long rightgreater   = (rightelements - ans[i]);
            long long  leftelements   = i;
