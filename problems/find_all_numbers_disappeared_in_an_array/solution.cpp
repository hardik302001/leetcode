...
public:
    vector<int> findDisappearedNumbers(vector<int>& nums) {
        ...
        {
            int n = abs(nums[i]); 
            if(nums[n-1] > 0)   // bcz if this condition do not exist then at end we will get nums[i]>0 for those numbers that are removed and repeated also, but we only want that are removed..
